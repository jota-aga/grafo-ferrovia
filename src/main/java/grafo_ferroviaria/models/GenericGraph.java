...
public class GenericGraph<V, E> {
    ...
    public Map<V, Double> dijkstra(
        ...
        while (!pq.isEmpty()) {
            ...
            Map<V, E> neigh = neighbors(u);
            if (neigh == null) continue;

            for (Map.Entry<V, E> entry : neigh.entrySet()) {
                V v = entry.getKey();
                E data = entry.getValue();

                if (allowed != null && !allowed.test(data)) continue;

                double w = cost.applyAsDouble(data);
                if (w < 0) throw new IllegalArgumentException("Dijkstra requires non-negative costs.");
